#!/usr/bin/env python3
"""
TESTE ABORDAGEM MODULAR
=======================

Testa a nova abordagem modular baseada nas sugestões do especialista:
1. Templates por bloco (abertura, desenvolvimento, conclusão)
2. Preenchimento com conteúdo dinâmico
3. Forçar inclusão de elementos autênticos
"""
...
def test_abordagem_modular():
    """Teste da nova abordagem modular baseada nas sugestões do especialista"""
    
    print("=" * 80)
    print("TESTE ABORDAGEM MODULAR")
    print("Implementação baseada nas sugestões do especialista")
    print("=" * 80)
    
    try:
        # Inicializar o agente
        print("\n1. INICIALIZANDO AGENTE COM ABORDAGEM MODULAR")
        print("-" * 60)
        writer = HumanizedWriter("config")
        print("SUCESSO - Agente inicializado com abordagem modular!")
        
        # Gerar matéria sobre MPC na IA
        print("\n2. GERANDO MATERIA COM ABORDAGEM MODULAR")
        print("-" * 60)
        topic = "MPC na IA"
        print(f"Tópico: {topic}")
        
        # Gerar a matéria usando o novo sistema modular
        materia = writer.humanize(topic, client_id="cliente1")
        print("Matéria gerada com sucesso usando abordagem modular!")
        ...
        print(f"\n3. ANALISANDO MATERIA GERADA COM ABORDAGEM MODULAR")
        print("-" * 60)
        print(f"Tamanho final: {len(materia)} caracteres")
        
        # Verificar elementos de autenticidade
        elementos_autenticos = {
            "Aberturas autênticas": ["Salve rapaziada", "E aí, galera", "Olha só, rapaziada", "Cara, galera", "Vou contar uma história"],
            "Histórias pessoais": ["amigo meu", "história verídica", "padaria", "café da manhã", "trabalhava com IA"],
            "Dados específicos": ["FGV", "Elon Musk", "Tesla", "10 milhões", "300 km", "5 bilhões", "Segundo dados da"],
        ...
        # Verificar se elementos autênticos foram incluídos
        elementos_incluidos = {
            "História pessoal": "amigo meu" in materia.lower() and "história verídica" in materia.lower(),
            "Dados específicos": any(word in materia.lower() for word in ["fvg", "elon musk", "tesla", "10 milhões"]),
            "Analogia": "casar com uma mulher feia" in materia.lower() or "china é uma autocracia" in materia.lower(),
            "Reflexão": "impressionante" in materia.lower() and "número de pessoas que me seguem" in materia.lower()
        }
        
        print(f"\nElementos autênticos incluídos:")
        for elemento, incluido in elementos_incluidos.items():
            status_icon = "[OK]" if incluido else "[FALTA]"
            print(f"- {elemento}: {status_icon}")
        
        # Mostrar a matéria gerada
        print(f"\n4. MATERIA GERADA COM ABORDAGEM MODULAR")
        print("-" * 60)
        print("=" * 80)
        print("MATÉRIA SOBRE MPC NA IA - ABORDAGEM MODULAR")
        print("=" * 80)
        print()
        print(materia)
        print()
        ...
        print(f"Elementos autênticos incluídos: {sum(elementos_incluidos.values())}/{len(elementos_incluidos)}")
        
        return materia, status == "SUCESSO", score_autenticidade, len(frases_problematicas), sum(elementos_incluidos.values())
...
if __name__ == "__main__":
    materia, sucesso, score, frases_malformadas, elementos_incluidos = test_abordagem_modular()
    if sucesso:
        print(f"\nOK - Abordagem modular funcionando perfeitamente!")
        print(f"Score de autenticidade: {score:.1f}%")
        print(f"Frases malformadas: {frases_malformadas}")
        print(f"Elementos autênticos incluídos: {elementos_incluidos}")
    else:
        print(f"\nATENCAO - Abordagem modular precisa de ajustes!")
